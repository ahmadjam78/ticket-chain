<template>
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200 bg-gray-50 flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">
                    User notifications
                    <span v-if="userId" class="font-mono text-indigo-600">(ID: {{ userId }})</span>
                </p>
            </div>
            <div v-if="hasUnread && userId" class="flex gap-2">
        ...
        <div class="p-6">
            <!-- Missing User ID -->
            <div v-if="!userId" class="text-center py-8 text-amber-600">
                <p>⚠️ No user selected. Please go back and choose a user.</p>
            </div>
...
const fetchNotifications = async (page = 1) => {
    if (!userId.value) {
        error.value = { message: 'No user ID provided' }
        return
    }
    ...
    error.value = null
    try {
        const response = await api.get(`/api/v1/admin/notifications/${userId.value}`, {
            params: { page }
        })
...
const markAsRead = async (id) => {
    ...
    isMarking.value = id
    try {
        await api.post(`/api/v1/admin/notifications/${id}/${userId.value}/mark-as-read`)
...
const markAllAsRead = async () => {
    ...
    try {
        await api.post(`/api/v1/admin/notifications/${userId.value}/mark-all-as-read`)
